...
export function ModelPicker({ models, selectedModelId, disabled, error, onSelect }: ModelPickerProps) {
	...
	return (
		<div className="relative min-w-0 max-w-[min(60vw,320px)] max-[620px]:max-w-[52vw]" ref={rootRef}>
			<TextButton
				className={`flex h-7 min-w-0 max-w-full items-center gap-1.5 px-2 text-xs text-muted hover:bg-toolbar-hover hover:text-foreground ${open ? 'bg-toolbar-hover text-foreground' : ''}`}
					...
					{[...providers].map(([providerName, providerModels]) => (
						<div key={providerName}>
							...
							{providerModels.map(model => (
								<TextButton
									className="grid min-h-8 w-full grid-cols-[16px_minmax(0,1fr)] items-center gap-1 px-1.5 py-1 text-left text-xs text-menu-foreground hover:bg-menu-selection aria-selected:[&_.model-check]:visible"
									key={model.id}
									role="option"
